...
import { ALL_REPAIRS_DEFAULT_COLUMNS, getDefaultAllRepairsColumnsState } from './allRepairsColumns';

type ManageAllRepairsColumnsDialogProps = {
  open: boolean;
  onClose: () => void;
  columnsState: AllRepairsColumnsState;
  onChange: (next: AllRepairsColumnsState) => void;
};

export function ManageAllRepairsColumnsDialog({
  open,
  onClose,
  columnsState,
  onChange,
}: ManageAllRepairsColumnsDialogProps) {
  ...
  const toggleVisibility = (id: AllRepairsColumnId) => {
    onChange({
      ...columnsState,
      visible: {
  ...
  const moveColumn = (id: AllRepairsColumnId, direction: 'up' | 'down') => {
  ...
  const handleReset = () => {
    onChange(getDefaultAllRepairsColumnsState());
  };
      ...
      <div className="manage-columns-dialog" ref={dialogRef}>
        ...
        <div className="manage-columns-body">
          {columnsState.order.map((columnId, index) => {
            const column = ALL_REPAIRS_DEFAULT_COLUMNS.find((c) => c.id === columnId);
