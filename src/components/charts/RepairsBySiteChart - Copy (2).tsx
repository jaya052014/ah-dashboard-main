...
import { ALL_REPAIRS_DATA as MOCK_DATA } from "../../data/allRepairsData";
// Jaya EOC
...
export function RepairsBySiteChart({
  ...
  const [allRepairs, setAllRepairs] = useState<any[]>(MOCK_DATA);
  const [loading, setLoading] = useState(true);
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
		...
		const liveData = jsonData.responseData['RRByPlant'];
        setAllRepairs(liveData);
  ...
  const ALL_REPAIRS_DATA = allRepairs;
  	// Jaya - EOC
  ...
  const { chartData, seriesKeys  } = useMemo(() => {
	...
	console.log('filtered', JSON.stringify(ALL_REPAIRS_DATA, null, 2));
	
      // Site filter
      const hasSiteFilter = selectedSites.length > 0 && !selectedSites.includes("all");
      if (hasSiteFilter) {
		  //console.log('insode hasSiteFilter', hasSiteFilter);
        const rowSite = repair.site || "";
        if (!selectedSites.includes(rowSite)) {
			//console.log('insode hasSiteFilter nested if');
          return false;
        }
      }
