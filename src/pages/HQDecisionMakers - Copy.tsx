...
// Jaya BOC
/*function getStatusSummaryDynamic(jsonData: JSONData, status: RepairStatus): { count: number; totalAmount: number } {
  //const filtered = repairs.filter((repair) => repair.status === status);
  const stats = JSONData.CountStatistics[0];;
  Object.entries(stats).map(([statusSummary, count]) => (
...
function getStatusSummaryDynamic(data, statusKey): { count: number; totalAmount: number } {
  //const filtered = repairs.filter((repair) => repair.status === status);
  
  //console.log('statusKey: ' , statusKey);
//console.log('data.CountStatistics: ', data?.responseData?.CountStatistics?.[0]);

const stats = data?.responseData?.CountStatistics?.[0];
//console.log('stats', stats);
  return {
    count: Number(stats[statusKey]) || 0,
    totalAmount: Number(stats[statusKey]) || 0,
  };
}
/*
const getStatusSummaryDynamic = (data, statusKey) => {
  // 1. Safety check to ensure data and the array exist
 // if (!data?.CountStatistics?.[0]) return 0;
 console.log('statusKey: ' , statusKey);
console.log('data.CountStatistics: ', data?.responseData?.CountStatistics?.[0]);
  ...
  const stats = data?.responseData?.CountStatistics?.[0];
console.log('stats', stats);
  // 3. Return the value for the key, or 0 if the key doesn't exist
...
//const fetchData = async () => {
	try {
	  ...
	  const jsonData = await response.json(); 
		...
		//console.log('hi json: ', JSON.stringify(jsonData, null, 2));
		...
		for (const status of statuses) {
							//console.log('inside for status: ', status);

			summaries[status] =  getStatusSummaryDynamic(jsonData, status);
			
			//console.log('inside for summaries[status] : ', status + summaries[status]);
		}
		console.log('summaries: b4 err: ', summaries);
		    return summaries as Record<RepairStatus, { count: number; totalAmount: number }>;
//console.log('after return: ', summaries);
	}	  
	catch (error) {
        //setError(error.message); // Set error state if fetching fails
        //setIsLoading(false); // Set loading to false
		console.log('err: ');
		return {} as Record<RepairStatus, { count: number; totalAmount: number }>;
      }	
	//}
...
export function HQDecisionMakers({ 
  ...
  // Modified by Jaya - BOC
  
  
  
  /**/
  
  var statusSummaries = useMemo(() => getAllStatusSummaries(allRepairsRows), [allRepairsRows]); // to review later
  
  
  var dynamicStatus = true; // to set from config file later
  if (dynamicStatus == true)
  {
	  console.log('hi');
	   statusSummaries = useMemo(() => getAllStatusSummariesDynamic(), [allRepairsRows]);
	   
	 /*  const [summaries, statusSummaries] = useState<Record<string, {count: number, totalAmount: number}> | null>(null);
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    const loadData = async () => {
      setLoading(true);
      const data = await getAllStatusSummariesDynamic();
      statusSummaries(data);
      setLoading(false);
    };
  ...
  // 2. Guard against 'undefined' while loading
  if (loading || !summaries) {
    return <div>Loading summaries...</div>;
  }*/
	   
      console.log('jaya1: truee ' , statusSummaries);

  }
  else 
  {
    statusSummaries = useMemo(() => getAllStatusSummaries(allRepairsRows), [allRepairsRows]);
		      console.log('jaya1: falseee ' , statusSummaries);

  }
	// Modified by Jaya - EOC
 //console.log('jaya');
  ...
  const statusKpiCards: Array<{
    ...
    return ALL_KPI_STATUSES.map((status) => ({
      ...
      status,
      value: statusSummaries[status]?.count ?? 0, //statusSummaries[status].count, // Jaya - BOC
	...
	//console.log('jaya: status ' , status);
	); 
  }, [statusSummaries]);
