...
// 1. Always declare hooks at the TOP level, outside any 'if' statements.
const [statusSummaries, setSummaries] = useState<Record<string, {count: number, totalAmount: number}>>({});
const [loading, setLoading] = useState(true);

// Change this to false if you want the old behavior
  ...
  const loadData = async () => {
    ...
    try {
      if (dynamicStatus) {
        ...
        // Option B: Get from local calculations
        const data = getAllStatusSummaries(allRepairsRows);
...
const statusKpiCards = useMemo(() => {
  ...
  return ALL_KPI_STATUSES.map((uiStatus) => {
    ...
    return {
      status: uiStatus,
      value: dataForStatus?.count ?? 0,
      IconComponent: REPAIR_STATUS_CONFIG[uiStatus]?.IconComponent ?? (() => null),
